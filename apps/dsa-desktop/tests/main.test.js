...
test("compareVersions follows semantic version ordering", (t) => {
  ...
  assert.equal(
    mainModule.compareVersions("3.13.0-beta.2", "3.13.0-beta.10"),
    -1,
  );
});
...
test("extractReleaseMetadata ignores releases without semver tags", (t) => {
  ...
  assert.equal(
    mainModule.extractReleaseMetadata({
      ...
      html_url:
        "https://github.com/ZhuLinsen/daily_stock_analysis/releases/tag/desktop-latest",
    }),
    null,
  );
});
...
test("auto download prompt falls back to error when install path fails", async (t) => {
  ...
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "dsa-desktop-updater-"),
  );
  ...
  let state = await mainModule.__getIpcMainHandler(
    "desktop:get-update-state",
  )();
  for (
    let idx = 0;
    idx < 12 && state.status !== mainModule.UPDATE_STATUS.ERROR;
    idx += 1
  ) {
  ...
  assert.equal(
    fs.existsSync(path.join(backupRoot, "runtime-state.json")),
    false,
  );
...
test("restorePackagedRuntimeStateFromBackup keeps backup when copy fails", (t) => {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "dsa-desktop-restore-"),
  );
...
test("restorePackagedRuntimeStateFromBackup removes restored files from pending manifest", (t) => {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "dsa-desktop-partial-restore-"),
  );
  const appDir = path.join(tempRoot, "app");
  const userDataDir = path.join(tempRoot, "userData");
  const backupRoot = path.join(userDataDir, ".dsa-desktop-update-backup");
  ...
  assert.deepEqual(JSON.parse(fs.readFileSync(manifestPath, "utf-8")).files, [
    dbRelativePath,
  ]);

  fs.writeFileSync(
    targetEnvPath,
    "user-change-after-partial-failure\n",
    "utf-8",
  );
  ...
  assert.equal(
    fs.readFileSync(targetEnvPath, "utf-8"),
    "user-change-after-partial-failure\n",
  );
  assert.deepEqual(JSON.parse(fs.readFileSync(manifestPath, "utf-8")).files, [
    dbRelativePath,
...
test("restorePackagedRuntimeStateFromBackup skips backup when app version did not change", (t) => {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "dsa-desktop-same-version-restore-"),
  );
  const appDir = path.join(tempRoot, "app");
  const userDataDir = path.join(tempRoot, "userData");
  const backupRoot = path.join(userDataDir, ".dsa-desktop-update-backup");
  ...
  fs.writeFileSync(
    targetEnvPath,
    "user-change-after-aborted-install\n",
    "utf-8",
  );
  fs.writeFileSync(
    manifestPath,
  ...
  assert.match(
    ...
    /stale backup target 3\.12\.0 was discarded/,
  );
  assert.equal(
    fs.readFileSync(targetEnvPath, "utf-8"),
    "user-change-after-aborted-install\n",
...
test("createWindow startup path does not throw ReferenceError after restore result handling", async (t) => {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "dsa-desktop-startup-"),
  );
  const appDir = path.join(tempRoot, "app");
  const userDataDir = path.join(tempRoot, "userData");
  ...
  fs.mkdirSync(userDataDir, { recursive: true });
  fs.mkdirSync(backupRoot, { recursive: true });
  fs.mkdirSync(path.join(resourcesPath, "backend", "stock_analysis"), {
    recursive: true,
  });
  fs.writeFileSync(exePath, "");
  fs.writeFileSync(uninstallPath, "");
  fs.writeFileSync(
    path.join(backupRoot, ".env"),
    "stale-backup-env\n",
    "utf-8",
  );
  fs.writeFileSync(
    manifestPath,
    JSON.stringify({ appVersion: "3.12.0", files: [".env"] }),
    "utf-8",
  );
  fs.writeFileSync(
    path.join(resourcesPath, "backend", "stock_analysis", "stock_analysis.exe"),
    "",
  );
  ...
  assert.equal(fs.existsSync(backupRoot), false);
  const updateState = await mainModule.__getIpcMainHandler(
    "desktop:get-update-state",
  )();
...
test("stopBackend waits for backend process exit", async (t) => {
  ...
  await Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("stopBackend did not resolve")), 200),
    ),
  ]);
