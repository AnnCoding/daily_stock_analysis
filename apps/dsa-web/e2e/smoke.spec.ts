...
async function login(page: Page) {
  test.skip(
    !smokePassword,
    "Set DSA_WEB_SMOKE_PASSWORD to run authenticated smoke tests.",
  );
...
test.describe("web smoke", () => {
  ...
  test("home page shows analysis entry and history panel after login", async ({
    page,
  }) => {
    await login(page);

    const stockInput = page.getByPlaceholder(
  ...
  test("chat page allows entering a question and starts a request", async ({
    page,
  }) => {
    await login(page);
    ...
    await expect(
      page.locator("p").filter({ hasText: prompt }).last(),
  ...
  test("settings page renders title and save actions after login", async ({
    page,
  }) => {
    await login(page);
  ...
  test("backtest page renders filter controls after login", async ({
    page,
  }) => {
    await login(page);
