...
async function login(page: Page) {
  test.skip(
    !smokePassword,
    "Set DSA_WEB_SMOKE_PASSWORD to run report markdown tests.",
  );
...
test.describe("ReportMarkdown component", () => {
  test("copy markdown source code", async ({ page, context }) => {
    ...
    const detailedReportButton = page.getByRole("button", {
      name: "完整分析报告",
    });
    ...
    await expect(
      page.getByRole("dialog").getByText("完整分析报告"),
    ).toBeVisible();

    // Click copy markdown button
    const copyMarkdownButton = page.getByRole("button", {
      name: "复制 Markdown 源码",
    });
    ...
    const checkmarkIcon = page.locator(
      'button[aria-label="复制 Markdown 源码"] svg.text-success',
    );
  ...
  test("copy plain text", async ({ page, context }) => {
    ...
    const copyPlainTextButton = page.getByRole("button", {
      name: "复制纯文本",
    });
    ...
    // Verify checkmark icon is shown
    const checkmarkIcon = page.locator(
      'button[aria-label="复制纯文本"] svg.text-success',
    );
    await expect(checkmarkIcon).toBeVisible();
