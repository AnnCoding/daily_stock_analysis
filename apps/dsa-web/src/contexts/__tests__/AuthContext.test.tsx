...
const { getStatus, login, changePassword, logout, resetDashboardState } =
  vi.hoisted(() => ({
    getStatus: vi.fn(),
    login: vi.fn(),
...
const Probe = () => {
  ...
  return (
    <div>
      ...
      <span data-testid="password-set">
        {auth.passwordSet ? "set" : "unset"}
      </span>
      <button
        type="button"
        onClick={() => void auth.login("passwd6", "passwd6")}
      >
        trigger-login
      </button>
