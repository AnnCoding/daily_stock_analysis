...
const RUNTIME_CAPABILITY_OPTIONS: Array<{
  value: LLMCapabilityCheck;
  label: string;
  hint: string;
}> = [
  {
    value: "json",
    label: "JSON",
    hint: "检测 response_format JSON 输出是否可用。",
  },
  {
    value: "tools",
    label: "Tools",
    hint: "检测 function/tool calling 是否可用。",
  },
  {
    value: "stream",
    label: "Stream",
    hint: "检测流式输出是否能返回有效 chunk。",
  },
  {
    value: "vision",
    label: "Vision",
    hint: "检测当前模型是否接受 image_url 输入。",
  },
];

const CAPABILITY_STATUS_LABELS: Record<
  LLMCapabilityCheckResult["status"],
  string
> = {
  passed: "通过",
  failed: "失败",
...
const ChannelRow: React.FC<ChannelRowProps> = ({
  ...
  const providerCapabilities = showProviderTemplateDetails
    ? preset?.capabilities || []
    : [];
  const providerSources = showProviderTemplateDetails
    ? preset?.officialSources || []
    : [];
  const providerHint = showProviderTemplateDetails
    ? preset?.configHint
    : undefined;
  ...
  const manualOnlyModels = selectedModels.filter(
    (model) =>
      !discoveredModels.some((discoveredModel) =>
        areModelsEquivalent(model, discoveredModel, channel.protocol),
      ),
  );
  ...
  return (
    <div className="mb-2 overflow-hidden rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface)] shadow-soft-card transition-[background-color,border-color,box-shadow] duration-200 hover:border-[var(--settings-border-strong)] hover:bg-[var(--settings-surface-hover)]">
      ...
      >
        <span
          className={`w-4 shrink-0 text-[11px] text-muted-text transition-transform ${expanded ? "rotate-90" : ""}`}
        >
          ▶
        </span>
        ...
        <span className="flex shrink-0 items-center gap-2">
          ...
          {!hasKey && channel.protocol !== "ollama" ? (
            <Badge variant="warning">未填 Key</Badge>
          ) : null}
        ...
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onChange={(v) =>
                  onUpdate(index, "protocol", normalizeProtocol(v))
                }
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {providerCapabilities.map((capability) => {
                  const capabilityMeta =
                    LLM_PROVIDER_CAPABILITY_LABELS[capability];
                  return (
                    <Tooltip key={capability} content={capabilityMeta.hint}>
                      <span className="inline-flex">
                        <Badge
                          variant="default"
                          className="border-[var(--settings-border)] bg-[var(--settings-surface)] text-secondary-text"
                        >
          ...
          <Input
            ...
            onPasswordVisibleChange={(nextVisible) =>
              onToggleKeyVisibility(index, nextVisible)
            }
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            ...
            <Input
              ...
              placeholder={
                preset?.placeholderModels ||
                MODEL_PLACEHOLDERS_BY_PROTOCOL[channel.protocol]
              }
              hint={
                discoveredModels.length > 0
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-[11px] font-medium text-muted-text">
                  运行时能力检测（可选）
                </p>
              ...
              <Button
                ...
                className="px-3 text-[11px] shadow-none"
                disabled={
                  busy || capabilityBusy || selectedCapabilities.length === 0
                }
                ...
                {RUNTIME_CAPABILITY_OPTIONS.map((option) => {
                  ...
                  return (
                    <Tooltip key={option.value} content={result.message}>
                      <span className="inline-flex">
                        <Badge
                          variant={getCapabilityResultVariant(result.status)}
                        >
                          {option.label}{" "}
                          {CAPABILITY_STATUS_LABELS[result.status]}
                        </Badge>
                      </span>
                    </Tooltip>
                  );
...
function toggleModelSelection(
  ...
  const index = selectedModels.findIndex((model) =>
    areModelsEquivalent(model, targetModel, protocol),
  );
  if (index >= 0) {
...
function normalizeModelForRuntime(
  ...
  if (trimmedModel.includes("/")) {
    ...
    if (
      KNOWN_MODEL_PREFIXES.has(lowerPrefix) ||
      KNOWN_MODEL_PREFIXES.has(canonicalPrefix)
    ) {
      if (
        canonicalPrefix !== lowerPrefix &&
        KNOWN_MODEL_PREFIXES.has(canonicalPrefix)
      ) {
...
function resolveModelPreview(
  ...
  return splitModels(models).map((model) =>
    normalizeModelForRuntime(model, protocol),
  );
}
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  ...
  timeout:
    "可重试；若持续超时，请检查 Base URL、网络代理、服务商可用区或本地防火墙。",
  quota: "请检查余额、套餐额度、RPM/TPM 限流或并发设置，必要时稍后重试。",
  model_not_found:
    "请确认模型名与渠道协议匹配，并先用“获取模型”核对该渠道实际可用模型列表。",
  empty_response:
    "渠道已连通但未返回正文；可尝试切换兼容模型、关闭额外响应模式后再测试。",
  network_error:
    "请检查 Base URL、代理、TLS/证书、中转网关或本地网络策略，并可稍后重试。",
  invalid_config: "先补齐协议、Base URL、API Key 和模型配置，再执行一键测试。",
  unsupported_protocol:
    "当前仅对 OpenAI Compatible / DeepSeek 渠道提供自动模型发现，请改为手动维护模型列表。",
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key:
    "API Key 为空，或逗号分隔后没有任何可用 Key；请填入至少一个有效 Key 后再测试。",
  api_key_rejected:
    "服务商拒绝了当前 API Key；请检查 Key、组织/项目权限、区域和账号状态。",
  rate_limit: "服务商触发 RPM/TPM 或并发限流；请降低请求频率或稍后重试。",
  insufficient_balance:
    "服务商返回余额、账单或额度不足；请检查账户余额和套餐状态。",
  quota_exceeded: "服务商返回配额已耗尽；请确认账号套餐、余量和项目额度。",
  provider_blocked:
    "请求被服务商或中转网关拦截；请检查账号风控、地域限制、模型权限、代理商网关策略、内容安全策略或请求来源限制。",
  dns_error: "域名解析失败；请检查 Base URL 域名、网络代理和 DNS 配置。",
  tls_error: "TLS/证书握手失败；请检查 HTTPS 证书、中转网关或公司代理策略。",
  connection_refused:
    "目标服务拒绝连接；请确认 Base URL 端口、服务进程和防火墙配置。",
  model_access_denied:
    "当前账号无法使用该模型；请确认模型是否已开通、账号是否可见，或模型是否已被禁用。",
  provider_prefix_mismatch:
    "模型 provider 前缀与当前渠道不匹配；请确认模型名是否应使用该渠道的 OpenAI-compatible 路由。",
...
function getLlmTroubleshootingHint(
  ...
  if (
    code === "empty_response" &&
    (context === "discovery" || stage === "model_discovery")
  ) {
...
function buildLlmTestHint(result: {
  ...
  const reason =
    typeof result.details?.reason === "string" ? result.details.reason : "";
  const detailsModel =
    typeof result.details?.model === "string" ? result.details.model : "";
  ...
  const shouldSuggestModelListChange =
  ...
  const modelActionHint = shouldSuggestModelListChange
    ...
    : "";
  const troubleshootingHint = getLlmTroubleshootingHint(
    result.errorCode,
    result.stage,
    "test",
    result.details,
  ...
  return (
    [modelHint, scopeInfo, modelActionHint, troubleshootingHint]
      .filter(Boolean)
      .join(" ") || undefined
  );
}
...
function getCapabilityResultVariant(
  status: LLMCapabilityCheckResult["status"],
...
function getFirstCapabilityHint(
  ...
  for (const result of Object.values(results)) {
    ...
    const hint = getLlmTroubleshootingHint(
      result.errorCode,
      result.stage,
      "test",
      result.details,
...
function hasLegacyRuntimeSource(
  model: string,
  itemMap: Map<string, string>,
): boolean {
  const provider =
    PROTOCOL_ALIASES[getRuntimeProvider(model)] || getRuntimeProvider(model);
  if (!provider || !MANAGED_PROVIDERS.has(provider)) {
    return false;
  }
  return (LEGACY_PROVIDER_KEYS[provider] || []).some(
    (key) => (itemMap.get(key) || "").trim().length > 0,
  );
}
...
function sanitizeRuntimeConfigForSave(
  ...
  const visionModel =
    ...
    !isRuntimeModelAvailable(
      ...
      ? ""
      : runtimeConfig.visionModel;
  const fallbackModels = runtimeConfig.fallbackModels.filter((model) =>
    isRuntimeModelAvailable(model, availableModels, itemMap),
  );
...
function runtimeConfigsAreEqual(
  left: RuntimeConfig,
  right: RuntimeConfig,
): boolean {
  return (
    left.primaryModel === right.primaryModel &&
...
function resolveTemperatureFromItems(itemMap: Map<string, string>): string {
  ...
  const provider = primaryModel.includes("/")
...
function parseRuntimeConfigFromItems(
  items: Array<{ key: string; value: string }>,
): RuntimeConfig {
  const itemMap = new Map(items.map((item) => [item.key, item.value]));
  return {
    primaryModel: itemMap.get("LITELLM_MODEL") || "",
    agentPrimaryModel: normalizeAgentPrimaryModel(
      itemMap.get("AGENT_LITELLM_MODEL") || "",
    ),
  ...
  return channelNames.map((name, index) => {
    ...
    return {
      ...
      protocol: inferProtocol(
        itemMap.get(`LLM_${upperName}_PROTOCOL`) || "",
        baseUrl,
        models,
      ),
      baseUrl,
      apiKey:
        itemMap.get(`LLM_${upperName}_API_KEYS`) ||
        itemMap.get(`LLM_${upperName}_API_KEY`) ||
        "",
      models: rawModels,
...
function channelsToUpdateItems(
  ...
  updates.push({
    key: "LLM_CHANNELS",
    value: channels.map((channel) => channel.name).join(","),
  });
  if (includeRuntimeConfig) {
    ...
    updates.push({
      ...
      value: runtimeConfig.fallbackModels.join(","),
    });
  ...
  for (const channel of channels) {
    ...
    updates.push({
      key: `${prefix}_API_KEY${isMultiKey ? "S" : ""}`,
      value: channel.apiKey,
    });
    updates.push({
      key: `${prefix}_API_KEY${isMultiKey ? "" : "S"}`,
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const initialNames = useMemo(
    () => initialChannels.map((channel) => channel.name),
    [initialChannels],
  );
  const initialRuntimeConfig = useMemo(
    () => parseRuntimeConfigFromItems(items),
    [items],
  );
  const savedItemMap = useMemo(
    () => new Map(items.map((item) => [item.key.toUpperCase(), item.value])),
    [items],
  );
  const hasLitellmConfig = useMemo(
    () =>
      items.some(
        (item) => item.key === "LITELLM_CONFIG" && item.value.trim().length > 0,
      ),
    [items],
  );
  ...
  const runtimeFingerprint = useMemo(
    () => JSON.stringify(initialRuntimeConfig),
    [initialRuntimeConfig],
  );

  const [channels, setChannels] = useState<ChannelConfig[]>(initialChannels);
  const [runtimeConfig, setRuntimeConfig] =
    useState<RuntimeConfig>(initialRuntimeConfig);
  ...
  const [testStates, setTestStates] = useState<
    Record<number, ChannelTestState>
  >({});
  const [discoveryStates, setDiscoveryStates] = useState<
    Record<string, ChannelDiscoveryState>
  >({});
  const [capabilityStates, setCapabilityStates] = useState<
    Record<string, ChannelCapabilityState>
  >({});
  ...
  const pendingSaveFeedbackFingerprintRef = useRef<{
    channels: string;
    runtime: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    prevChannelsRef.current = channelsFingerprint;
    prevRuntimeRef.current = runtimeFingerprint;
    const pendingSaveFeedbackFingerprint =
      pendingSaveFeedbackFingerprintRef.current;
    const preserveSaveFeedback =
      pendingSaveFeedbackFingerprint?.channels === channelsFingerprint &&
  ...
  const availableModels = useMemo(() => {
    ...
    for (const channel of channels) {
      ...
      for (const model of resolveModelPreview(
        channel.models,
        channel.protocol,
      )) {
      ...
      previous.map((channel, rowIndex) => {
        ...
        if (field === "name" && typeof value === "string") {
          ...
          if (newPreset) {
            ...
            if (
              !updated.baseUrl ||
              updated.baseUrl === (oldPreset?.baseUrl ?? "")
            ) {
            ...
            if (
              !updated.models ||
              updated.models === (oldPreset?.placeholderModels ?? "")
            ) {
  ...
  const addChannel = () => {
    const preset =
      getProviderTemplate(addPreset) || getProviderTemplate("custom");
    if (!preset) {
      return;
    }
    setChannels((previous) => {
  ...
  const handleSave = async () => {
    ...
    if (managesRuntimeConfig) {
      ...
      const invalidAgentPrimaryModel =
      ...
      const invalidFallbackModel = runtimeConfigForSave.fallbackModels.some(
        (model) =>
          !isRuntimeModelAvailable(model, availableModels, savedItemMap),
      );
      if (invalidFallbackModel) {
        setSaveMessage({
          type: "local-error",
          text: "存在无效的备选模型，请重新选择。",
        });
        return;
      }

      const invalidVisionModel =
    ...
    try {
      ...
      setSaveMessage({
        type: "success",
        text: managesRuntimeConfig ? "AI 配置已保存" : "渠道配置已保存",
      });
    } catch (error: unknown) {
      setSaveWarnings([]);
  ...
  const handleCapabilityCheck = async (channel: ChannelConfig) => {
    ...
    try {
      ...
      const hasFailure = Object.values(capabilityResults).some(
        (item) => item?.status === "failed",
      );
      const hasSkipped = Object.values(capabilityResults).some(
      ...
      setCapabilityStates((previous) => ({
        ...
        [channel.id]: {
          selected,
          status:
            hasFailure || hasSkipped || !result.success ? "error" : "success",
          text:
            Object.keys(capabilityResults).length > 0
              ...
              : result.success
                ...
                : buildLlmFailureText(result),
          hint:
            getFirstCapabilityHint(capabilityResults) ||
  ...
  const setPrimaryModel = (value: string) => {
    setRuntimeConfig((previous) => ({
      ...previous,
      primaryModel: value,
      fallbackModels: previous.fallbackModels.filter(
        (model) => model !== value,
      ),
    }));
  };
      ...
      >
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="info" className="settings-accent-badge">
              渠道管理
            </Badge>
          </div>
          <p className="text-xs text-muted-text">
            添加服务商渠道后可自动获取模型列表并多选，也可继续手动填写。配置会自动同步到
            .env 文件。
          </p>
        </div>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Badge
                variant="default"
                className="border-[var(--settings-border)] bg-[var(--settings-surface-hover)] text-muted-text"
              >
                {channels.length} 个渠道
              </Badge>
            </div>
            ...
            {channels.length === 0 ? (
              ...
              channels.map((channel, index) => (
                <ChannelRow
                  ...
                  onDiscoverModels={(channel) =>
                    void handleDiscoverModels(channel)
                  }
                  onToggleCapability={toggleCapability}
                  onCheckCapabilities={(channel) =>
                    void handleCapabilityCheck(channel)
                  }
                />
              ))
            )}
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <span className="settings-accent-text text-xs font-medium uppercase tracking-wider">
                    运行时参数
                  </span>
                  <p className="mt-1 text-[11px] text-muted-text">
                    主模型、备选模型、Vision 与 Temperature
                    会直接写入运行时配置。
                  </p>
                </div>
                <Badge
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="w-8 text-right text-sm text-secondary-text">
                ...
                <div className="space-y-4">
                  <div>
                    <label
                      ...
                      className="mb-1 block text-xs text-muted-text"
                    >
                      主模型
                    </label>
                    <Select
                    ...
                    <div className="space-y-2 rounded-xl border settings-border-strong settings-surface-overlay-soft p-3">
                      {availableModels.map((model) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={runtimeConfig.fallbackModels.includes(
                              model,
                            )}
                            disabled={
                              busy || model === runtimeConfig.primaryModel
                            }
