...
export const SettingsCategoryNav: React.FC<SettingsCategoryNavProps> = ({
  ...
  return (
    <div className="h-full rounded-[1.5rem] border settings-border bg-card/94 p-4 shadow-soft-card-strong backdrop-blur-sm">
      <div className="mb-4">
        <p className="settings-accent-text text-xs font-semibold uppercase tracking-[0.3em]">
          配置分类
        </p>
      ...
      <div className="space-y-2.5">
        {categories.map((category) => {
          ...
          const description = getCategoryDescriptionZh(
            category.category,
                  ...
                  {description ? (
                    <p
                      className={cn(
                        ...
                        isActive ? "text-secondary-text" : "text-muted-text",
                      )}
                    >
                      {description}
                    </p>
