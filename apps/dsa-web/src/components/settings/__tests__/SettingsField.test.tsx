...
describe("SettingsField", () => {
  ...
  it("allows optional select fields to be cleared when schema provides an empty option", () => {
    ...
    expect(
      screen.queryByRole("option", { name: "请选择" }),
  ...
  it("renders localized custom webhook body template guidance", () => {
    ...
    expect(
      screen.getByLabelText("自定义 Webhook Body 模板"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/会先于 Bark、Slack、Discord 等自动 payload 生效/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/裸 \$content \/ \$title 不做 JSON 转义/),
    ).toBeInTheDocument();
  });
  ...
  it("opens detailed field help when help metadata is available", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "查看 自选股列表 配置说明" }),
    );

    expect(
      screen.getByRole("dialog", { name: "自选股列表" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("STOCK_LIST=600519,300750,002594"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("dialog", { name: "自选股列表" }),
    ).not.toBeInTheDocument();
  });
});
