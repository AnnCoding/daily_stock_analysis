...
describe("llmProviderTemplates", () => {
  it("keeps provider template order aligned with the existing preset dropdown order", () => {
    expect(
      LLM_PROVIDER_TEMPLATES.map((template) => template.channelId),
    ).toEqual([
      "aihubmix",
      "anspire",
      "deepseek",
  ...
  it("derives lookup keys from unique channel ids", () => {
    const channelIds = LLM_PROVIDER_TEMPLATES.map(
      (template) => template.channelId,
    );
  ...
  it("exposes safe helpers for known, custom, and unknown channel ids", () => {
    expect(getProviderTemplate("openrouter")).toBe(
      LLM_PROVIDER_TEMPLATE_BY_ID.openrouter,
    );
    expect(getProviderTemplate("custom")).toBe(
  ...
  it("keeps focused config hints on providers with common setup pitfalls", () => {
    expect(LLM_PROVIDER_TEMPLATE_BY_ID.ollama.configHint).toContain(
      "Ollama 服务",
    );
    expect(LLM_PROVIDER_TEMPLATE_BY_ID.siliconflow.configHint).toContain(
      "API Key",
    );
    expect(LLM_PROVIDER_TEMPLATE_BY_ID.openrouter.configHint).toContain(
      "API Key",
    );
  ...
  it("keeps basic metadata on non-custom provider templates", () => {
    for (const template of LLM_PROVIDER_TEMPLATES.filter(
      (item) => item.channelId !== "custom",
    )) {
