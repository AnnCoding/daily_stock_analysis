...
const { update, testLLMChannel, discoverLLMChannelModels } = vi.hoisted(() => ({
  update: vi.fn(),
...
vi.mock("../../../api/systemConfig", () => ({
  systemConfigApi: {
    ...
    discoverLLMChannelModels: (...args: unknown[]) =>
      discoverLLMChannelModels(...args),
  },
}));
...
describe("LLMChannelEditor", () => {
  ...
  it("hides LiteLLM wording when advanced YAML routing is enabled", () => {
    ...
    expect(
      screen.getByText(/检测到已配置高级模型路由 YAML/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /运行时主模型 \/ 备选模型 \/ Vision \/ Temperature 仍由下方通用字段决定/i,
      ),
    ).toBeInTheDocument();
  ...
  it("keeps minimax-prefixed models in runtime selections", () => {
    ...
    const visionModelSelect = screen.getByRole("combobox", {
      name: "Vision 模型",
    });

    expect(
      within(primaryModelSelect).getByRole("option", {
        name: "minimax/MiniMax-M1",
      }),
    ).toBeInTheDocument();
    expect(
      within(agentModelSelect).getByRole("option", {
        name: "minimax/MiniMax-M1",
      }),
    ).toBeInTheDocument();
    expect(
      within(visionModelSelect).getByRole("option", {
        name: "minimax/MiniMax-M1",
      }),
    ).toBeInTheDocument();
  });
  ...
  it.each([
    ...
    [
      ...
      "doubao-seed-1-6-251015,doubao-seed-1-6-thinking-251015",
    ],
  ])(
    "uses %s OpenAI-compatible defaults when adding the official preset",
    async (preset, buttonName, baseUrl, models) => {
      render(
        <LLMChannelEditor
          items={[]}
          configVersion="v1"
  ...
  it("shows provider capability badges, official sources, and config hints", async () => {
    ...
    expect(
      screen.getByText(/模型列表和模型可见性依赖账号权限与 API Key/i),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "OpenRouter Models API" }),
    ).toHaveAttribute(
      "href",
      "https://openrouter.ai/docs/api/api-reference/models/get-models",
    );
    expect(
      screen.getByText(/能力标签仅用于配置参考，不代表运行时能力已验证通过/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows model-discovery capability for SiliconFlow provider hints", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "SiliconFlow Models" }),
    ).toBeInTheDocument();
  });
  ...
  it("does not show provider metadata for custom or unknown channels", async () => {
    ...
    expect(
      screen.queryByText(/能力标签仅用于配置参考/i),
  ...
  it("uses the selected preset defaults when adding a duplicate provider channel", async () => {
    ...
    expect(
      screen
        .getAllByLabelText("渠道名称")
        .map((input) => (input as HTMLInputElement).value),
    ).toEqual(["minimax", "minimax2"]);
    expect(
      screen
        .getAllByLabelText("Base URL")
        .map((input) => (input as HTMLInputElement).value),
    ).toEqual(["https://api.minimax.io/v1", "https://api.minimax.io/v1"]);
    expect(
      screen
        .getAllByLabelText("模型（逗号分隔）")
        .map((input) => (input as HTMLInputElement).value),
    ).toEqual([
      "MiniMax-M2.7,MiniMax-M2.7-highspeed",
      "MiniMax-M2.7,MiniMax-M2.7-highspeed",
    ...
    expect(
      screen.getAllByRole("link", { name: "MiniMax OpenAI API" }),
    ).toHaveLength(1);
  });
    ...
    expect(updatePayload.items).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          key: "LLM_MINIMAX_PROTOCOL",
          value: "openai",
        }),
        expect.objectContaining({
          key: "LLM_MINIMAX_BASE_URL",
          value: "https://api.minimax.io/v1",
        }),
        expect.objectContaining({
          key: "LLM_MINIMAX_MODELS",
  ...
  it("sanitizes stale runtime models before saving DeepSeek V4 channel changes", async () => {
      ...
      <LLMChannelEditor
        items={[
          ...
          { key: "AGENT_LITELLM_MODEL", value: "deepseek/deepseek-reasoner" },
          {
            ...
            value:
              "deepseek/deepseek-v4-pro,deepseek/deepseek-chat,cohere/command-r-plus",
          },
    ...
    expect(updatePayload.items).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          key: "LITELLM_FALLBACK_MODELS",
          value: "deepseek/deepseek-v4-pro,cohere/command-r-plus",
        }),
    ...
    render(
      <LLMChannelEditor
        items={[
          ...
          {
            key: "LLM_DEEPSEEK_MODELS",
            value: "deepseek-chat,deepseek-v4-pro",
          },
          { key: "LITELLM_MODEL", value: "deepseek/deepseek-chat" },
  ...
  it("shows cleanup warning and restore path after stale runtime models are removed on save", async () => {
    ...
    render(
      <LLMChannelEditor
        items={[
          ...
          {
            key: "LITELLM_FALLBACK_MODELS",
            value: "deepseek/deepseek-v4-pro,deepseek/deepseek-chat",
          },
          { key: "VISION_MODEL", value: "deepseek/deepseek-reasoner" },
        ]}
        configVersion="v1"
    ...
    expect(
      screen.getByText(/已同步清理失效的运行时模型引用/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/桌面端导出备份或手动 \.env 还原/i),
    ).toBeInTheDocument();
  });

  it("keeps save warnings visible after onSaved-driven refresh", async () => {
    const warningMessage =
      "检测到已同步清理失效的运行时模型引用：主模型 / Agent 主模型 / Vision 模型 / 备选模型中的失效项。";
    const initialItems = [
      { key: "LLM_CHANNELS", value: "deepseek" },
      { key: "LLM_DEEPSEEK_PROTOCOL", value: "deepseek" },
      { key: "LLM_DEEPSEEK_BASE_URL", value: "https://api.deepseek.com" },
    ...
    render(
      <LLMChannelEditor
        items={[
          ...
          {
            key: "LLM_DEEPSEEK_MODELS",
            value: "deepseek-chat,deepseek-reasoner,deepseek-v4-pro",
          },
          { key: "LITELLM_MODEL", value: "deepseek/deepseek-chat" },
          { key: "AGENT_LITELLM_MODEL", value: "deepseek/deepseek-reasoner" },
          { key: "LITELLM_FALLBACK_MODELS", value: "deepseek/deepseek-v4-pro" },
  ...
  it("keeps direct-env provider runtime models (cohere / google / xai) while saving channel changes", async () => {
      ...
      <LLMChannelEditor
        items={[
          ...
          { key: "AGENT_LITELLM_MODEL", value: "google/gemini-2.5-flash" },
          {
            key: "LITELLM_FALLBACK_MODELS",
            value:
              "cohere/command-r-plus,google/gemini-2.5-flash,xai/grok-beta",
          },
    ...
    expect(updatePayload.items).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          key: "LITELLM_FALLBACK_MODELS",
          value: "cohere/command-r-plus,google/gemini-2.5-flash,xai/grok-beta",
        }),
        expect.objectContaining({
          key: "VISION_MODEL",
  ...
  it("discovers models and writes selected values back to channel config", async () => {
    ...
    expect(updatePayload.items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          key: "LLM_DASHSCOPE_MODELS",
          value: "qwen-old,qwen-plus",
        }),
      ]),
    );
  });
  ...
  it("shows structured troubleshooting hint when channel auth fails", async () => {
    ...
    expect(
      await screen.findByText(
        /聊天调用 · 鉴权失败：LLM authentication failed/i,
      ),
    ).toBeInTheDocument();
    expect(screen.getByText(/请检查 API Key 是否正确/i)).toBeInTheDocument();
    expect(
      screen.queryByText(/调整模型顺序或移除不可用模型/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows tested model and model-availability hints when a model is disabled", async () => {
    ...
    render(
      <LLMChannelEditor
        items={[
          ...
          {
            key: "LLM_SILICONFLOW_MODELS",
            value: "deepseek-ai/DeepSeek-V3,Qwen/Qwen3-Coder",
          },
        ]}
        configVersion="v1"
        maskToken="******"
    ...
    expect(
      await screen.findByText(
        /聊天调用 · 模型不可用：LLM channel test failed/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/本次测试模型：openai\/deepseek-ai\/DeepSeek-V3/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /基础连接测试默认使用模型列表首项：deepseek-ai\/DeepSeek-V3/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/基础连接测试默认只测试模型列表中的第一个模型/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/调整模型顺序或移除不可用模型/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/模型是否已开通、账号是否可见/i),
    ).toBeInTheDocument();
    expect(screen.queryByText(/Base URL、代理、TLS/i)).not.toBeInTheDocument();
    expect(testLLMChannel).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("shows provider blocked troubleshooting without network or model-list hints", async () => {
    ...
    expect(
      await screen.findByText(/聊天调用 · 请求被拦截/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/本次测试模型：openai\/gpt-5\.5/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/账号风控、地域限制、模型权限/i),
    ).toBeInTheDocument();
    expect(screen.queryByText(/Base URL、代理、TLS/i)).not.toBeInTheDocument();
    expect(
      screen.queryByText(/调整模型顺序或移除不可用模型/i),
  ...
  it("shows focused quota exceeded troubleshooting hints", async () => {
    ...
    expect(
      await screen.findByText(/服务商返回配额已耗尽/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/调整模型顺序或移除不可用模型/i),
    ).not.toBeInTheDocument();
  ...
  it("does not request runtime capabilities during the basic connection test", async () => {
    ...
    expect(testLLMChannel).toHaveBeenCalledWith(
      expect.not.objectContaining({ capabilityChecks: expect.anything() }),
    );
  });
  ...
  it("runs explicit runtime capability checks and shows detailed hints", async () => {
    ...
    expect(
      await screen.findByText(/能力检测完成：1 通过 \/ 1 失败 \/ 0 跳过/i),
    ).toBeInTheDocument();
    expect(screen.getByText("JSON 通过")).toBeInTheDocument();
    expect(screen.getByText("Tools 失败")).toBeInTheDocument();
    expect(
      screen.getByText(/当前模型或兼容层不支持该能力/i),
    ).toBeInTheDocument();
    expect(testLLMChannel).toHaveBeenCalledWith(
      expect.objectContaining({ capabilityChecks: ["json", "tools"] }),
    );
  });
  ...
  it("shows skipped runtime capabilities when the base test fails", async () => {
    ...
    render(
      <LLMChannelEditor
        items={[
          ...
          { key: "LLM_OPENAI_BASE_URL", value: "https://api.openai.com/v1" },
          { key: "LLM_OPENAI_ENABLED", value: "true" },
          { key: "LLM_OPENAI_API_KEY", value: "bad-key" },
          { key: "LLM_OPENAI_MODELS", value: "gpt-4o-mini" },
        ]}
        configVersion="v1"
        maskToken="******"
    ...
    expect(
      await screen.findByText(/能力检测完成：0 通过 \/ 0 失败 \/ 1 跳过/i),
    ).toBeInTheDocument();
  ...
  it("keeps manual model input available when discovery fails", async () => {
    ...
    await screen.findByText(
      /模型发现 · 协议暂不支持：Model discovery is not supported for this protocol/i,
    );
    expect(
      screen.getByText(
        /当前仅对 OpenAI Compatible \/ DeepSeek 渠道提供自动模型发现/i,
      ),
    ).toBeInTheDocument();
  ...
  it("maps discovery format errors to the /models troubleshooting hint", async () => {
    ...
    expect(
      await screen.findByText(
        /响应解析 · 格式异常：Failed to parse \/models response/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /该渠道返回的 \/models 响应格式不兼容，请改为手动填写模型列表。/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("maps discovery empty responses to the /models troubleshooting hint", async () => {
    ...
    expect(
      await screen.findByText(
        /模型发现 · 空响应：No model IDs returned from \/models response/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/该渠道的 \/models 接口未返回可用模型 ID/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/切换兼容模型、关闭额外响应模式/i),
    ).not.toBeInTheDocument();
  });
