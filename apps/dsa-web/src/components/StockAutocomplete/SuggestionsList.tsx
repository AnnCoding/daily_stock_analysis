        ...
        <li
          ...
          className={cn(
            ...
            index === highlightedIndex &&
              "bg-[var(--autocomplete-hover-bg)]/25",
          )}
...
const MARKET_BADGE_CONFIG = {
  ...
  INDEX: {
    label: "指数",
    className: "border-purple/25 bg-purple/10 text-purple",
  },
  ETF: {
    label: "ETF",
...
function MarketBadge({ market }: { market: string }) {
  ...
  return (
    <Badge
      ...
      size="sm"
      className={cn(
        "min-w-[3rem] justify-center shadow-none",
        config.className,
      )}
    >
      {config.label}
...
function MatchTypeBadge({ matchType }: { matchType: string }) {
  ...
  return (
    <Badge
      variant="default"
      size="sm"
      className={cn("shrink-0 shadow-none", config.className)}
    >
      {config.label}
    </Badge>
  );
