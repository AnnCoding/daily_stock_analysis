...
const settingsHelpZhCN: SettingsHelpMap = {
  "settings.base.STOCK_LIST": {
    title: "自选股列表",
    summary:
      "配置需要分析的股票代码列表，是手动分析、定时任务和通知报告的基础输入。",
    usage:
      "多个股票代码使用英文逗号分隔。A 股可直接填写 6 位代码，港股可使用 hk 前缀，美股可填写 ticker。",
    valueNotes: [
  ...
  "settings.ai_model.LITELLM_MODEL": {
    ...
    usage:
      "推荐使用 provider/model 格式，例如 deepseek/deepseek-v4-flash、gemini/gemini-3.1-pro-preview 或 ollama/qwen3:8b。",
    valueNotes: [
  ...
  "settings.ai_model.LLM_CHANNELS": {
    title: "LLM 渠道列表",
    summary:
      "声明多个模型渠道，用于多 provider、多 Key、备用模型和可视化渠道管理。",
    usage:
      "填写逗号分隔的渠道名，例如 deepseek,aihubmix；每个渠道再配置 LLM_<NAME>_BASE_URL、LLM_<NAME>_API_KEY(S)、LLM_<NAME>_MODELS 等字段。",
    valueNotes: [
      "启用渠道模式后，同层运行时优先读取渠道配置。",
      "在 Docker 或 GitHub Actions 中显式注入的环境变量会覆盖 Web 设置页写入的 .env。",
      "渠道编辑器保存时只更新本次提交的 key，不会静默迁移整个旧配置。",
    ],
    impact: ["影响主模型、Agent 模型、fallback 模型和 Vision 模型的可选来源。"],
    notes: [
  ...
  "settings.notification.FEISHU_WEBHOOK_URL": {
    ...
    usage:
      "在飞书群中添加自定义机器人后，复制 open-apis/bot/v2/hook 开头的 Webhook URL 到这里。",
    valueNotes: [
    ...
    impact: ["影响飞书通知渠道；失败时不应拖垮主分析流程，只影响该渠道送达。"],
    notes: [
  ...
  "settings.system.WEBUI_HOST": {
    ...
    usage:
      "本机访问通常使用 127.0.0.1；云服务器、Docker 或需要外部访问时通常使用 0.0.0.0。",
    valueNotes: [
    ...
    impact: ["影响重启后浏览器能否从本机、局域网或公网访问 WebUI。"],
    notes: [
...
const settingsHelpEnUS: SettingsHelpMap = {
  "settings.base.STOCK_LIST": {
    title: "Watchlist",
    summary:
      "Defines the stock codes used by analysis jobs and notification reports.",
    usage:
      "Separate symbols with commas. A-shares can use six-digit codes, HK stocks can use the hk prefix, and US stocks can use ticker symbols.",
    valueNotes: [
    ...
    impact: [
      "Affects analysis scope, notification content, and saved history reports.",
    ],
    notes: [
      "Use English commas between symbols.",
  ...
  "settings.ai_model.LITELLM_MODEL": {
    ...
    usage:
      "Use provider/model format, such as deepseek/deepseek-v4-flash, gemini/gemini-3.1-pro-preview, or ollama/qwen3:8b.",
    valueNotes: [
    ...
    impact: [
      "Affects regular stock analysis, market review, report generation, and Agent calls without a dedicated model.",
    ],
    notes: [
  ...
  "settings.ai_model.LLM_CHANNELS": {
    title: "LLM Channels",
    summary:
      "Declares model channels for multiple providers, keys, fallbacks, and visual channel management.",
    usage:
      "Use comma-separated names such as deepseek,aihubmix; then configure LLM_<NAME>_BASE_URL, LLM_<NAME>_API_KEY(S), and LLM_<NAME>_MODELS for each channel.",
    valueNotes: [
    ...
    impact: [
      "Affects available sources for primary, Agent, fallback, and Vision models.",
    ],
    notes: [
  ...
  "settings.notification.FEISHU_WEBHOOK_URL": {
    ...
    summary:
      "Sends analysis reports to a Feishu group through a custom bot webhook.",
    usage:
      "Create a custom bot in the target Feishu group and paste the open-apis/bot/v2/hook webhook URL here.",
    valueNotes: [
    ...
    impact: [
      "Affects only the Feishu notification channel; delivery failure should not block the main analysis flow.",
    ],
    notes: [
  ...
  "settings.system.WEBUI_HOST": {
    ...
    usage:
      "Use 127.0.0.1 for local-only access. Use 0.0.0.0 for cloud, Docker, or external access.",
    valueNotes: [
    ...
    impact: [
      "Affects whether the WebUI can be reached locally, on the LAN, or from the public internet after restart.",
    ],
    notes: [
...
export function getSettingsHelpContent(
  ...
  const localized =
    getPreferredHelpMap(locale)[helpKey] ?? settingsHelpZhCN[helpKey];
  if (localized) {
    return localized;
  }
