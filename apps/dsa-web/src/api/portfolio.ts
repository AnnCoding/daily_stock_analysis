...
function buildFxRefreshParams(
  query: FxRefreshQuery,
): Record<string, string | number> {
  const params: Record<string, string | number> = {};
  if (query.accountId != null) {
    params.account_id = query.accountId;
...
export const portfolioApi = {
  ...
  async refreshFx(
    query: FxRefreshQuery = {},
  ): Promise<PortfolioFxRefreshResponse> {
    const response = await apiClient.post<Record<string, unknown>>(
  ...
  async deleteTrade(tradeId: number): Promise<PortfolioDeleteResponse> {
    const response = await apiClient.delete<Record<string, unknown>>(
      `/api/v1/portfolio/trades/${tradeId}`,
    );
  ...
  async deleteCashLedger(entryId: number): Promise<PortfolioDeleteResponse> {
    const response = await apiClient.delete<Record<string, unknown>>(
      `/api/v1/portfolio/cash-ledger/${entryId}`,
    );
    return toCamelCase<PortfolioDeleteResponse>(response.data);
  },
  ...
  async deleteCorporateAction(
    actionId: number,
  ): Promise<PortfolioDeleteResponse> {
    const response = await apiClient.delete<Record<string, unknown>>(
      `/api/v1/portfolio/corporate-actions/${actionId}`,
    );
    return toCamelCase<PortfolioDeleteResponse>(response.data);
  },
  ...
  async listCashLedger(
    query: CashListQuery = {},
  ): Promise<PortfolioCashLedgerListResponse> {
    const params = buildEventParams(query);
    if (query.direction) {
