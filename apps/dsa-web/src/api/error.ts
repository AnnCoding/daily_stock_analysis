...
export function createParsedApiError(
  options: CreateParsedApiErrorOptions,
): ParsedApiError {
  return {
    title: options.title,
...
export function getParsedApiError(error: unknown): ParsedApiError {
  ...
  if (
    isRecord(error) &&
    isParsedApiError((error as ErrorCarrier).parsedError)
  ) {
...
export function createApiError(
  ...
  const apiError = new Error(formatParsedApiError(parsed)) as Error &
    ErrorCarrier & {
      status?: number;
      category: ApiErrorCategory;
      rawMessage: string;
...
export function parseApiError(error: unknown): ParsedApiError {
  ...
  const hasStockCodeField = includesAny(matchText, [
    "stock_code",
    "stock_codes",
  ]);
  const hasMissingParamText = includesAny(matchText, [
  ...
  if (
    errorCode === "portfolio_oversell" ||
    includesAny(matchText, ["oversell detected"])
  ) {
    return createParsedApiError({
      title: "卖出数量超过可用持仓",
  ...
  if (
    errorCode === "portfolio_busy" ||
    includesAny(matchText, ["portfolio ledger is busy"])
  ) {
    return createParsedApiError({
      title: "持仓账本正忙",
  ...
  if (
    ...
    return createParsedApiError({
      title: "服务端无法访问外部依赖",
      message:
        "页面已连接到本地服务，但本地服务访问外部模型或数据接口失败，请检查代理、DNS 或出网配置。",
      rawMessage,
      status,
  ...
  if (localConnectionFailed) {
    return createParsedApiError({
      title: "无法连接到本地服务",
      message:
        "浏览器当前无法连接到本地 Web 服务，请检查服务是否启动、监听地址是否正确、端口是否开放。",
      rawMessage,
      status,
