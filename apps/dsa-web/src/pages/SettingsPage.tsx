...
function getDesktopUpdateNotice(
  ...
  if (state.status === "error") {
    return {
      ...
      actionLabel:
        state.updateMode === "auto" && state.releaseUrl
          ? "前往下载"
          : undefined,
      actionKind:
        state.updateMode === "auto" && state.releaseUrl ? "release" : undefined,
    };
  }
...
const SettingsPage: React.FC = () => {
  ...
  const [envBackupActionError, setEnvBackupActionError] =
    useState<ParsedApiError | null>(null);
  const [envBackupActionSuccess, setEnvBackupActionSuccess] =
    useState<string>("");
  ...
  const handleEnvBackupImportFile = async (
                  ...
                  <div className="mt-4 space-y-3 rounded-2xl border settings-border bg-background/30 px-4 py-4">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div>
                        ...
                        <p className="text-xs leading-6 text-muted-text">
                          启动后会自动检查 GitHub Releases 最新正式版；Windows
                          安装版会后台下载更新并提示重启安装。
                        </p>
                      </div>
                      <Button
                      ...
                      <SettingsAlert
                        ...
                        onAction={
                          desktopUpdateNotice.actionLabel
                            ? () => {
                                if (
                                  desktopUpdateNotice.actionKind === "install"
                                ) {
            ...
            {activeCategory === "system" ? (
              ...
              >
                <div className="space-y-4">
                  {!isEnvBackupAllowed ? (
                    <p className="text-xs leading-6 text-amber-700 dark:text-amber-300">
                      当前 Web 端未开启管理员鉴权，导出/导入 `.env`
                      备份功能已停用；请先将 `ADMIN_AUTH_ENABLED` 设为 `true`
                      并完成管理员登录后再使用。
                    </p>
                  ) : null}
                  ...
                  {envBackupActionError ? (
                    <ApiErrorAlert
                      ...
                      actionLabel={
                        envBackupActionError.status === 409
                          ? "重新加载"
                          : undefined
                      }
                      onAction={
