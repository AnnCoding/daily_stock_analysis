...
describe("searchStocks", () => {
  ...
  test("activeOnly 选项过滤不活跃股票", () => {
    ...
    for (const result of results) {
      const item = mockIndex.find(
        (i) => i.canonicalCode === result.canonicalCode,
      );
  ...
  test("sorts by popularity when scores are tied", () => {
    ...
    if (results.length > 1) {
      for (let index = 0; index < results.length - 1; index++) {
        const currentItem = mockIndex.find(
          (item) => item.canonicalCode === results[index].canonicalCode,
        );
        const nextItem = mockIndex.find(
          (item) => item.canonicalCode === results[index + 1].canonicalCode,
        );
        if (results[index].score === results[index + 1].score) {
          expect(currentItem?.popularity || 0).toBeGreaterThanOrEqual(
            nextItem?.popularity || 0,
          );
        }
      }
    }
  ...
  describe("Inactive stock tests", () => {
    ...
    test("active stocks prioritized over inactive stocks", () => {
      ...
      if (results.length > 1) {
        ...
        const firstItem = mockIndex.find(
          (i) => i.canonicalCode === results[0].canonicalCode,
        );
  ...
  describe("Performance tests", () => {
    test("large index search performance", () => {
      ...
      const largeIndex: StockIndexItem[] = Array.from(
        { length: 5000 },
        (_, i) => ({
          canonicalCode: `${i}.SH`,
          displayCode: `${i}`,
