...
describe("cn utility", () => {
  ...
  it("should handle conditional classes", () => {
    ...
    const isFalse = false;
    expect(
      cn("base-class", isTrue && "active-class", isFalse && "hidden-class"),
  ...
  it("should preserve custom extracted component classes alongside utilities", () => {
    ...
    expect(cn("terminal-card border-subtle", "border-primary/50")).toBe(
      "terminal-card border-primary/50",
    );
    expect(cn("glass-card p-4", "p-6")).toBe("glass-card p-6");
    expect(cn("btn-primary bg-blue-500", "bg-red-500")).toBe(
