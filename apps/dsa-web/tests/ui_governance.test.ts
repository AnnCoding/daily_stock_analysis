...
const nativeTitlePattern =
  /<(button|a|input|textarea|select|div|span)\b[^>]*\btitle=/;
...
describe("web UI governance guards", () => {
  it("does not reintroduce native title attributes on common interactive elements", () => {
    const violations = sourceFiles.filter((filePath) =>
      nativeTitlePattern.test(readFileSync(filePath, "utf8")),
    );
  ...
  it("keeps input-terminal out of application source", () => {
    const violations = sourceFiles.filter((filePath) =>
      inputTerminalPattern.test(readFileSync(filePath, "utf8")),
    );
    expect(violations).toEqual([]);
  });
});
