...
"""
        if context.get('is_open_end_fund'):
            prompt += """
> ⚠️ **开放式基金分析约束**：该标的为开放式基金（非上市交易型）。
> - 仅有每日净值（NAV）数据，无 OHLCV、成交量、换手率
> - 严禁分析量能配合、筹码结构、龙虎榜、换手率等交易维度
> - 严禁输出 RSI、KDJ、MACD、乖离率、量比、布林带等基于交易数据的技术指标，这些对基金无意义
> - `volume_analysis` 固定输出"开放式基金无成交量数据"
> - `chip_structure` 固定输出"开放式基金无筹码分布数据"
> - `risk_alerts` 禁止使用"超买"、"超卖"、"RSI高位"、"量价背离"等交易型术语
> - `positive_catalysts` 禁止使用量能放大、资金流入等交易型描述
> - `signal_type` 使用"🟢持有/定投信号"而非"买入信号"
> - `battle_plan` 中不设止损位、目标位等短线交易点位，改为：定投/分批买入/赎回建议
> - `support_level`/`resistance_level` 留空（净值无支撑阻力概念）
> - 重点关注：基金经理、持仓结构、规模变动、申购赎回状态、费率、业绩比较基准、同类排名
> - 技术分析仅基于净值走势（MA均线趋势），忽略所有成交量相关信号
